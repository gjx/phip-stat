...
def submit_to_LSF(queue,wait,LSFopfile,cmd_to_submit,mem_usage=None):
    # wrap command to submit in quotations
    cmd_to_submit = r'"%s"' % cmd_to_submit.strip(r'"')
    LSF_params = {'LSFoutput':LSFopfile,
                       'wait':wait,
                      'queue':queue}
    LSF_cmd = 'bsub -q%(queue)s -W%(wait)s -o%(LSFoutput)s' % LSF_params
    if mem_usage != None:
...
argparser.add_argument('-W','--wait',required=True)
...
for infilename in glob.glob(os.path.join(input_dir,'*.csv')):
    ...
    outfilename = os.path.join(output_dir,'.'.join([sample,'csv']))
    ...
    print submit_to_LSF(args.queue,args.wait,logfilename,cmd,mem_usage=args.mem_usage)
